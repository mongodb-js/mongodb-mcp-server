...
export class StreamsBuildTool extends StreamsToolBase {
    ...
    private static extractConnectionNames(obj: unknown): Set<string> {
        const names = new Set<string>();
        if (Array.isArray(obj)) {
            for (const item of obj) {
                for (const name of StreamsBuildTool.extractConnectionNames(item)) {
                    names.add(name);
                }
            }
            ...
            for (const [key, value] of Object.entries(record)) {
                if (key === "connectionName" && typeof value === "string") {
                    ...
                    for (const name of StreamsBuildTool.extractConnectionNames(value)) {
                        names.add(name);
                    }
                }
            }
    ...
    private async validatePipelineConnections(
        ...
        const referencedNames = StreamsBuildTool.extractConnectionNames(pipeline);
