...
export class StreamsDiscoverTool extends StreamsToolBase {
    ...
    private async getNetworking(
        ...
        const [privateLinkResult, vpcPeeringResult] = await Promise.allSettled([
            this.apiClient.listPrivateLinkConnections({
                params: { path: { groupId: projectId } },
            }),
            this.apiClient.listActivePeeringConnections({
                params: { path: { groupId: projectId } },
            }),
        ]);
        ...
        if (vpcPeeringResult.status === "fulfilled" && vpcPeeringResult.value) {
            const peeringData = vpcPeeringResult.value as Record<string, unknown>;
            const results = (peeringData.results ?? peeringData) as unknown[];
            if (Array.isArray(results) && results.length > 0) {
                sections.push(`## Active VPC Peering Connections\n${JSON.stringify(results, null, 2)}`);
            } else {
                sections.push("## Active VPC Peering Connections\nNo active VPC peering connections found.");
            }
        } else {
            sections.push("## Active VPC Peering Connections\nNo active VPC peering connections found.");
        }
