...
- **Don't rely on `npx -y github:<owner>/mongodb-mcp-server#<branch>` for first-time launch.** A cold install (clone + `npm install` + `tsc` build) of this dependency tree takes 90–120s — well over Claude Desktop's ~60s startup timeout and Claude Code's default 30s `MCP_TIMEOUT`. If you want the npx route anyway, pre-warm the cache once from a terminal where a Node satisfying the engine range is active — `npx -y github:<owner>/mongodb-mcp-server#<branch> --help` — before starting the client, and pass `MCP_TIMEOUT=180000` to `claude` for the first run. Subsequent launches reuse the warm cache and start in ~5s. Otherwise, build locally and use the snippet above. Note that `npx` itself is a `#!/usr/bin/env node` script, so even `"command": "/abs/path/to/npx"` re-resolves `node` from PATH.
