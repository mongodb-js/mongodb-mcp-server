...
| `MDB_MCP_HTTP_ALLOWED_HOSTS` / `--httpAllowedHosts`                                            | `""`                                                                                                                                               | Additional values accepted in the HTTP Host header beyond the loopback defaults and the bound httpHost/httpPort (only used when transport is 'http'). Set this to allow reverse-proxy hostnames or, when binding to a non-loopback address, the externally reachable host. Each entry may include a port, e.g. 'mcp.example.com' or 'mcp.example.com:8443'. |
| `MDB_MCP_HTTP_ALLOWED_ORIGINS` / `--httpAllowedOrigins`                                        | `""`                                                                                                                                               | Additional values accepted in the browser Origin header beyond the loopback defaults, the bound httpHost/httpPort, and httpAllowedHosts (only used when transport is 'http'). Entries may be full origins such as 'https://app.example.com' or host[:port] values.                                                                                          |
