...
| ([preview](#opting-into-preview-features)) `voyageApiKey` | `MDB_VOYAGE_API_KEY`                                | <not set>                                                                   | API key for communicating with Voyage AI. Used for generating embeddings for Vector search. **This feature is in private preview and requires opting into the `vectorSearch` preview feature**.         |
