...
- `atlas-upgrade-cluster` - Upgrade or scale a MongoDB Atlas cluster: upgrades Free/Flex clusters to Flex or M10 Dedicated, or scales an already-Dedicated cluster's size/autoscaling. Note to LLM: ask for provider and region together up front if unknown. Common region mappings by provider (default recommendation: AWS US_EAST_1):
