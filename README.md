...
- `connect` - Connect to a MongoDB instance and get back a `connectionId` to pass to the other MongoDB tools. When the server is started with a configured connection string, a connection with the id `preconfigured` is available without calling this tool.
...
- `list-connections` - List the active MongoDB connections and their `connectionId`s
