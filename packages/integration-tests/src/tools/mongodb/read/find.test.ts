...
describeWithMongoDB({
    name: "find tool with default configuration",
    fn: (integration) => {
        validateToolMetadata({
            integration,
            name: "find",
        ...
        it("returns 0 when collection doesn't exist", async () => {
            ...
            expect(content).toEqual(
                'Query on collection "non-existent" resulted in 0 documents. Returning 0 documents.'
            );
            expectFindStructuredContent(response, content, { count: 0, documents: [] });
        });
...
for (const { suiteLabel, userConfig, cases } of findLimitSuites) {
    describeWithMongoDB({
        name: `find tool with ${suiteLabel}`,
        fn: (integration) => {
            beforeEach(async () => {
                await freshInsertDocuments({
                    collection: integration.mongoClient().db(integration.randomDbName()).collection("foo"),
...
describeWithMongoDB({
    name: "find tool with abort signal",
    fn: (integration) => {
...
describeWithMongoDB({
    name: "find tool with configured maxTimeMS",
    fn: (integration) => {
        beforeEach(async () => {
            await freshInsertDocuments({
                collection: integration.mongoClient().db(integration.randomDbName()).collection("foo"),
    ...
    config: {
        getUserConfig: () => ({ ...defaultTestConfig, maxTimeMS: 10000 }),
    },
});
describeWithMongoDB({
    name: "find tool with low maxTimeMS rejects slow queries",
    fn: (integration) => {
        beforeEach(async () => {
            await freshInsertDocuments({
                collection: integration.mongoClient().db(integration.randomDbName()).collection("foo"),
...
describeWithMongoDB({
    name: "find tool with server-side JavaScript operators",
    fn: (integration) => {
        afterEach(() => {
