        ...
        describe("with existing database", () => {
            it("creates new collection", async () => {
                ...
                expect(content).toEqual(
                    `Collection "collection2" created in database "${integration.randomDbName()}".`
                );
                // Validate structured content
                const structuredContent = response.structuredContent as CreateCollectionOutput;
                expect(structuredContent.database).toBe(integration.randomDbName());
            ...
            it("does nothing if collection already exists", async () => {
                ...
                const content = getResponseContent(response.content);
                expect(content).toEqual(
                    `Collection "collection1" created in database "${integration.randomDbName()}".`
                );
                // Validate structured content
                const structuredContent = response.structuredContent as CreateCollectionOutput;
                expect(structuredContent.database).toBe(integration.randomDbName());
