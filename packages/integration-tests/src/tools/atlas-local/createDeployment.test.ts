...
const SAMPLE_DATA_TEST_TIMEOUT_MS = 600000;
describeWithAtlasLocal({
    ...
    fn: (integration) => {
        ...
        it("should create a deployment with voyageApiKey set when preview image is used", async () => {
            ...
            ).toBe(configWithVoyageApiKey.voyageApiKey);
        });
        it(
            ...
            async () => {
                ...
                const connectionId = await vi.waitFor(
                    ...
                    { timeout: SAMPLE_DATA_TEST_TIMEOUT_MS, interval: 5000 }
                );
    ...
    options: { config: configWithVoyageApiKey },
});
describeWithAtlasLocalDisabled({
    name: "[MacOS in GitHub Actions] atlas-local-create-deployment",
    fn: (integration) => {
