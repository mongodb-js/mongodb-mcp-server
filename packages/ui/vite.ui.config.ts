...
/**
 * Writes the per-entry mount module for a UI module and returns its path.
 *
 * Each mount statically imports exactly ONE widget. A shared mount with an
 * eager `import.meta.glob` registry defeated tree-shaking: every HTML bundle
 * contained every widget plus the whole mcp-ui/LeafyGreen dependency graph
 * (~2 MB supersets differing by a few bytes).
 */
function writeEntryMount(set: UiSet, moduleName: string): string {
    const entryKey = set.entryFileName(moduleName).replace(/\.html$/, "");
    const mountFilePath = join(entriesDir, `${entryKey}.mount.tsx`);
    const widgetPath = resolve(configDir, "src", set.srcDir, moduleName, "index.js");
    const fontsPath = resolve(configDir, "src/styles/fonts.css");
    // The mcp-ui components historically ship the bundled Euclid @font-face
    // rules. MCP App widgets are sandboxed with the no-network default CSP
    // (no csp declared on the resource), where those remote faces can never
    // load — their typography is the host font variable with a system-ui
    // fallback — so they get no font CSS.
    const fontsImport = set.srcDir === "components" ? `import ${JSON.stringify(fontsPath)};\n` : "";

    writeFileSync(
        mountFilePath,
        `// Auto-generated by vite.ui.config.ts (generate-html-entries) - do not edit.
${fontsImport}import React from "react";
import { createRoot } from "react-dom/client";
import { ${moduleName} } from ${JSON.stringify(widgetPath)};
 ...
 * based on the template.html file, plus the per-entry mount module that
 * imports only that widget.
 */
...
export default defineConfig(
    (): UserConfig => ({
        ...
        plugins: [
            generateHtmlEntries(),
            // Retained although no module in either widget graph imports
            // buffer/stream: transitive dependencies may reference the Buffer
            // global without importing it, and the injected shims measure only
            // a few KB in the bundles.
            nodePolyfills({
