 ...
 * Source: packages/compass-explain-plan/src/components/explain-tree/tree-layout.tsx
 * Copyright MongoDB, Inc. Original license: SSPL-1.0 (MongoDB-internal reuse).
 *
 * Deviation: emotion `css` from @mongodb-js/compass-components replaced with
 * inline styles.
 */
...
interface TreeLayoutProps<T, X> extends Omit<React.HTMLProps<HTMLDivElement>, "data" | "children"> {
    data: T;
...
interface LinkPathProps<T> {
    ...
    linkColor: string;
    arrowColor: string;
    getLinkWidth: (sourceNodeData: T, targetNodeData: T, metaData?: unknown) => number;
}
...
function LinkPath<T>({
    ...
    arrowColor,
    getLinkWidth,
}: LinkPathProps<T>): React.JSX.Element {
...
function TreeLayout<T, X>({
    ...
    scale = 1,
    ...divProps
}: TreeLayoutProps<T, X>): React.JSX.Element {
        ...
        <div {...divProps}>
            <div
                style={{
                    ...
                    // center position of the container we are adjusting container size to
                    // match the scale
                    //
