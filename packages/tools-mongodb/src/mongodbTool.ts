        ...
        if (error instanceof MongoDBError) {
            switch (error.code) {
                ...
                case ErrorCodes.MisconfiguredConnectionString: {
                    const outcome = await this.session.connectionErrorHandler(error, {
                        availableTools: this.session.availableTools,
                ...
                case ErrorCodes.AtlasSearchNotSupported: {
                    const CTA = this.session.isToolCategoryAvailable("atlas-local")
                        ? "`atlas-local` tools"
                        : "Atlas CLI";
                    return {
                        content: [
