...
const componentsModulePath = resolve(__dirname, "src/ui/build/components.generated.ts");
...
function discoverComponents(): { components: string[]; toolToComponentMap: Record<string, string> } {
    ...
    const entries = readdirSync(componentsDir).sort((a, b) => a.localeCompare(b));
...
/**
 * Vite plugin that generates a deterministic components registry module.
 * This avoids non-deterministic import.meta.glob ordering.
 */
function generateComponentRegistry(): Plugin {
    return {
        name: "generate-component-registry",
        buildStart() {
            const imports = components
                .map((componentName) => {
                    return `import { ${componentName} } from "../components/${componentName}/index.js";`;
                })
                .join("\n");

            const entries = components.map((componentName) => `    ${componentName}: ${componentName},`).join("\n");
...
`;
            ...
            writeFileSync(componentsModulePath, registryContent);
            console.log("[generate-component-registry] Generated components registry");
        },
    };
}
...
function generateUIModule(): Plugin {
    return {
        ...
        closeBundle() {
            ...
            const toolEntries = Object.entries(toolToComponentMap).sort(([a], [b]) => a.localeCompare(b));
            for (const [toolName, componentName] of toolEntries) {
